...
# Modified for yielding image matrix and class_index

from __future__ import print_function
from __future__ import division

import numpy as np
from core.dataset import Dataset, IterableDataset
...
class BatchSampler(object):
    ...
    def _read_data(self, batch_indices):
        if self.which is None:
            imgs = np.stack((self.dataset[i] for i in batch_indices), axis=0)
            return imgs
        elif self.which == 'source':
            imgs = np.stack((self.dataset[i][0] for i in batch_indices), axis=0)
            idx = np.stack((self.dataset[i][1] for i in batch_indices), axis=0)
            return [imgs, np.array(idx)]
        elif self.which == 'reference':
            imgs = np.stack((self.dataset[i][0] for i in batch_indices), axis=0)
